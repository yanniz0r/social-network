...
const friendshipMutationResolvers: MutationResolvers = {
  async acceptFriendshipRequest(_parent, { id }, context) {
    ...
    if (!friendship) throw new ApolloError(`Could not find friendship with id ${id}`)
    if (friendship.acceptedAt) throw new ApolloError(`Friendship ${id} has already been accepted at ${friendship.acceptedAt.toISOString()}`)
