      ...
      <div className="p-10">
        {meQuery.data?.me && (
          <>
              ...
              <div className="ml-5">
                ...
                {meQuery.data.me.birthday && (
                  <div className="flex items-center text-sm dark:text-gray-400">
                    ...
                    <span className="ml-1">
                      Geboren am{" "}
                      {format(new Date(meQuery.data.me.birthday), "dd.MM.yyyy")}{" "}
                      (
                      {differenceInYears(
                        new Date(),
                        new Date(meQuery.data.me.birthday)
                      )}{" "}
                      Jahre alt)
                    </span>
                  </div>
            ...
            <div className="mt-4">
              <Card>
                <h2 className="px-5 pt-5 pb-2 dark:text-gray-200 text-lg">
                  Freunde
                </h2>
                <ul>
                  {meQuery.data.me.friends.map((friend) => (
                    <li key={friend.id} className="flex px-5 py-3 items-center">
                      <div className="w-10 h-10 bg-blue-400 flex items-center justify-center text-white rounded-full">
                        {friend.name[0]}
                      </div>
