...
import { useFriendshipsPageQuery, useFriendsPageAcceptFriendshipRequestMutation as useAcceptFriendshipMutation } from "../graphql/generated";
  ...
  return <div className="p-10">
    {Boolean(friendsPageQuery.data?.friendshipRequests.length) &&
      <Card className="py-3 px-5 mb-5">
        <h2 className="text-xl dark:text-white mb-2">Freundschaftsanfragen</h2>
        <ul>
          {friendsPageQuery.data?.friendshipRequests.map(friendshipRequest => (
            <li key={friendshipRequest.from.id}>
              <Link passHref href={`/profile/${friendshipRequest.from.id}`}>
                <a className="flex py-2">
                  <Avatar size="md" name={friendshipRequest.from.name} online={friendshipRequest.from.online} />
                  <div className="ml-2 flex items-center flex-grow">
                    <div className="text-md dark:text-gray-300">{friendshipRequest.from.name}</div>
                  </div>
                  <div className="flex">
                    ...
                    <div className="w-2" />
                    <IconButton color="green" onClick={() => acceptFriendship(friendshipRequest.id)}>
                      <FaUserPlus />
                    </IconButton>
                  </div>
                </a>
          ...
          <li key={friend.id}>
            <Link passHref href={`/profile/${friend.id}`}>
              <a className="flex py-2">
                ...
                <div className="ml-2 flex items-center flex-grow">
                  <div className="text-md dark:text-gray-300">{friend.name}</div>
                </div>
              </a>
            </Link>
          </li>
