...
const Search: FC<SearchProps> = (props) => {
  ...
  const clearButtonClassName = classNames("absolute h-full px-3 right-0 top-0 transition-all transform scale-0 opacity-0 text-gray-400", {
  ...
  return <div className="rounded-lg relative">
    <div className={containerClassName}>
      <div className="relative">
        <input placeholder="Suchen" className="bg-gray-600 px-3 py-2 text-md rounded-lg w-full block text-white" value={query} onChange={e => setQuery(e.target.value)} />
                    ...
                    {searchQuery.data?.searchUsers.map(user => (
                      <li key={user.id}>
                        <Link passHref href={`/profile/${user.id}`}>
                          <a className="flex items-center px-2 py-2" onClick={clear}>
                            <Avatar name={user.name} imageURL={user.avatarURL ?? undefined} size="md" />
                            <div className="ml-2">
                              <span className="font-bold block text-gray-800">{user.name}</span>
                              <span className="text-gray-600 text-sm block">{user.birthday}</span>
                            </div>
                          </a>
                        </Link>
                      </li>
