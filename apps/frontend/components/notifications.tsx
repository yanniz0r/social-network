...
import { useNotificationsNewNotificationSubscription, useNotificationsQuery } from "../graphql/generated"
...
const Notifications: FC<NotificationsProps> = (props) => {
  ...
  const containerClassName = classNames("w-full h-full absolute left-0 top-0 bg-gray-800 transition-all bg-opacity-90 backdrop-blur", {
  ...
  return <div className={containerClassName}>
    <div className="p-4 text-center text-white border-b-2 border-gray-700 relative">
      Neuigkeiten
      <a className="absolute left-0 top-0 h-full flex items-center p-2 cursor-pointer" onClick={props.close}>
        <FaChevronLeft />
      </a>
    </div>
    {notificationsQuery.data?.notifications.map(notification => (
      <div key={notification.id} className="p-2">
        ...
        {notification.__typename == 'PostLikedNotification' &&
          <NotificationRow date={new Date(notification.date)} imageURL={notification.liker.avatarURL ?? undefined} linkURL="/">
